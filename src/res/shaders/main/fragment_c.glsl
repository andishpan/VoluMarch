...
void getLighting(vec3 position, vec3 normal, vec3 reflectionDir, vec3 rayDirection, int materialID, inout vec3 color) {
    ...
    if (materialID == WATER_MATERIAL_ID) {
        ...
        vec3 reflectedDir = reflect(rayDirection, waterNormal);
        ...
        float F0 = 0.02;
        float fresnel = getFresnel(waterNormal, -rayDirection, F0);
...
vec3 raymarch(in vec3 rayOrigin, in vec3 rayDirection, out vec3 outvolumeColor) {
...
//check water
    if (rayDirection.y != 0.0) {
        ...
        if (tPlane > EPSILON && tPlane < closestT) {
            closestT = tPlane;
...
// raymarch inside volume =>
    if (volumetricDepth < nearestHitT) {
        for (int i = 0; i < uMaxVolumeSteps; i++) {
            ...
            if (sdfValue < 0.0) {
                ...
                float prevsunTransmittance = transmittance;
                ...
                float marchAbsorption = prevsunTransmittance - transmittance;
               ...
               // if (i % 10 == 0) {
                    ...
                    for (int j = 0; j < uMaxLightMarchSteps; j++) {
                        ...
                        if (getVolume(currentLightPoint) < 0.0) {
                            lightTransmittance *= BeerLambert(uVolumetricAbsorption, marchSize * 1.4);
               ...
               // }
                ...
                volumeColor += marchAbsorption * uVolumetricAlbedo * ambientColor;
            }
        }
    }
    ...
    if (hitOpaque && materialId != INVALID_MATERIAL_ID) {
        ...
        getLighting(position, normal, reflectionDir, rayDirection, materialId, surfaceColor);
    } else {
...
vec3 raymarchForwardScattering(in vec3 rayOrigin, in vec3 rayDirection, out vec3 outvolumeColor) {
    vec3 volumeColor       = vec3(0.0);
    vec3 surfaceColor       = vec3(0.0);
    float nearestHitT      = SCENE_MAX_T;
    float transmittance = 1.0;
        ...
        for (int i = 0; i < uMaxVolumeSteps; i++) {
            ...
            if (sdfValue < 0.0) {
              ...
              //  }
                ...
                volumeColor += scatterLight * uVolumetricAlbedo * lightCol;
                volumeColor += marchAbsorption * uVolumetricAlbedo * ambientColor;
            }
        }
    }
...
vec3 raymarchBackwardScattering(in vec3 rayOrigin, in vec3 rayDirection, out vec3 outvolumeColor) {
    vec3 volumeColor       = vec3(0.0);
    vec3 surfaceColor       = vec3(0.0);
    float nearestHitT      = SCENE_MAX_T;
    float transmittance = 1.0;
...
vec3 raymarchMOS(in vec3 rayOrigin, in vec3 rayDirection, out vec3 outvolumeColor){
    vec3 volumeColor       = vec3(0.0);
    vec3 surfaceColor       = vec3(0.0);
    float nearestHitT      = SCENE_MAX_T;
    float transmittance = 1.0;
        ...
        for (int octave = 0; octave < NUM_SCATTER_OCTAVES; octave++) {
            ...
            for (int i = 0; i < uMaxVolumeSteps; i++) {
                ...
                if (sdfValue < 0.0) {
                    ...
                    transmittance *= BeerLambert(uVolumetricAbsorption * fog, localStep);
                    ...
                    if (!isColorTooDark(lightCol)) {
                      ...
                      //  if (i % 6 == 0) {
                       ...
                       // }
                        ...
                        lightCol *= lightTransmittance * scatter;
                    }

                    vec3 scatterContribution = marchAbsorption * uVolumetricAlbedo * lightCol;
                    volumeColor += amplitude * scatterContribution;


                    volumeColor += amplitude * marchAbsorption * uVolumetricAlbedo * ambientColor;
                }
            }
...
vec3 raymarchDual(in vec3 rayOrigin, in vec3 rayDirection, out vec3 outvolumeColor){
    vec3 volumeColor       = vec3(0.0);
    vec3 surfaceColor       = vec3(0.0);
    float nearestHitT      = SCENE_MAX_T;
    float transmittance = 1.0;
                    ...
                    if (!isColorTooDark(lightCol)) {
                      ...
                      //  if (i % 6 == 0) {
                       ...
                       // }
                        ...
                        lightCol *= lightTransmittance * phase;
                    }
...
void main(){
    ...
    if (uCurrentMethod == 0) {
        color = raymarch(rayOrigin, rayDirection, volumeColor);
    }
    else if (uCurrentMethod == 1) {
        color = raymarchMOS(rayOrigin, rayDirection, volumeColor);
    }
    else if (uCurrentMethod == 2) {
        color = raymarchBackwardScattering(rayOrigin, rayDirection, volumeColor);
    }
    else if (uCurrentMethod == 3){
        color = raymarchForwardScattering(rayOrigin, rayDirection, volumeColor);
    }else{
        color = raymarchDual(rayOrigin, rayDirection, volumeColor);
    }
