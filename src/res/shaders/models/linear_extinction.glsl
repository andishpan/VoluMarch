    ...
    if (volumetricDepth < nearestHitT) {
        for (int i = 0; i < uMaxVolumeSteps; i++) {
            ...
            if (sdfValue < 0.0) {
                ...
                float k = uVolumetricAbsorption * density;
                float stepTransmittance = LinearExtinction(k, uStepSize);
                ...
                shadowTransmittance = shadowMarchLinear(p, lightDir, uStepSize * 1.4, uMaxLightMarchSteps);
