    ...
    if (volumetricDepth < nearestHitT) {
        for (int i = 0; i < uMaxVolumeSteps; i++) {
            ...
            if (sdfValue < 0.0) {
                ...
                float s = uVolumetricAbsorption * density;
                float stepTransmittance = SchlickExtinction(s, uStepSize);
                ...
                shadowTransmittance = shadowMarchSchlick(p, lightDir, uShadowStepSize, uMaxLightMarchSteps);
